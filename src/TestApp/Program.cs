...
namespace TestApp
{
    class Program
    {
		...
		private const String AppMasterSecret = "YOUR APP SECRET";
	    
		private const String TestDeviceToken = "YOUR_TEST_DEVICE_DEVICE_TOKEN";
        private const String TestNamedDevice = "YOUR NAMED USER";
		...
		private static void TestPush()
		{
            ...
            var user = new Audience(AudienceType.NamedUser, TestNamedDevice);

            Console.WriteLine("PUSH to NamedUser" + TestNamedDevice);
            response = _urbanAirSharpGateway.Push("Broadcast Alert",new List<DeviceType>() { DeviceType.Ios, DeviceType.Android },null,null,user);
            Console.Write(response.HttpResponseCode + " - ");
            Console.WriteLine(response.Ok ? "SUCCESS" : "FAILED");
            Console.WriteLine();
        }
